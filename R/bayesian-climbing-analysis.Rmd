...
```{r convergence-diagnostics, echo=FALSE}
print("MCMC settings:")
print(data$mcmc.settings)
print("Convergence diagnostics for m:")
print(data$diagnostics$m)
print(paste0("R-hat range across all parameters: [",
             signif(data$diagnostics$rhat.range[1], 3), ", ",
             signif(data$diagnostics$rhat.range[2], 3), "]"))
print(paste0("Minimum n_eff: ", signif(data$diagnostics$neff.min, 3),
             "; median n_eff: ", signif(data$diagnostics$neff.median, 3)))
print(paste0("Divergent transitions: ", data$diagnostics$n.divergent,
             "; iterations hitting max treedepth: ", data$diagnostics$n.maxtreedepth))
```
...
# Save diagnostics in a separate small file so they survive even when the full
# stanfit object is discarded.
diag.file.name <- paste0(data$file.stem, "-diagnostics.rds")
