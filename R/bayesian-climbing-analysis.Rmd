...
params:
  ...
  min.ascents: 400
  min.failures: 1
  in.path: "~/Git/climbing/data/processed/"
  out.path: "~/Git/climbing/results/aus/"
...
The average slope from a simple regression of the log-odds of failure is `r mean(data$m)`
...
The final processed data set includes a total of `r data$d$N` ascents by `r data$d$C` climbers over `r data$d$P` months. 

Not all climbers made ascents during all months. The total number of climber months is `r sum(data$d$maxPage - data$d$minPage + 1)`.

The average number of ascents per climber per active month is `r data$d$N / sum(data$d$maxPage - data$d$minPage + 1)`.
