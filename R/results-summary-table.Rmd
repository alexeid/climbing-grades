...
summary.row <- function(df, data, country) {
  ...
  data.frame(country=country, gear=paste0(data$params$gear, collapse="+"), climbers=data$d$C, ascents=data$d$N, slope=slope,hpd.lower=hpd.lower, hpd.upper=hpd.upper, min.ascents=data$params$min.ascents, min.failures=data$params$min.failures, game=game, time=time.in.seconds)
}
...
all.rows <- function(df, data, country) {
  ...
  data.frame(country=country, gear=gear, game=game, slope=slope, central=central)
}
...
kable(summary.table[order(summary.table$slope, decreasing=T),])

```
```{r}

label <- paste(summary.table$country, summary.table$gear, summary.table$game)
summary.table$label <- label

label <- paste(full.table$country, full.table$gear, full.table$game)
...
levels <- summary.table$label[order(as.character(summary.table$gear), as.character(summary.table$country), summary.table$game)]

full.table$label <- factor(full.table$label,levels = levels,ordered = TRUE)

p <- full.table[full.table$central & full.table$game == "attempt",] %>%
