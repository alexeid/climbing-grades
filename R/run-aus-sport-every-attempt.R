#!/usr/bin/env Rscript
# Launch the AUS Sport per-session analysis on the "every-attempt logger" subset
# (filter.by.tickprop.lrp = TRUE: <=5% generic ticks AND <=5% lonely redpoints).
# This addresses reviewer comment #9 (the "data not shown" claim).
...
params <- list(
  ...
  endDate              = "2021-08-01",
  filter.by.tickprop.lrp = TRUE,   # the every-attempt-logger filter
...
cat("=== Configuration row", i, "(every-attempt-logger subset) ===\n")
print(params)
...
data$params <- params
cat("Climbers (post-filter):", data$d$C, "  Ascents:", data$d$N, "  Months:", data$d$P, "\n")

cat("\n=== Running Stan inference ===\n")
options(mc.cores = parallel::detectCores())
stan.result <- run.stan.climbing.model(data$d,
  sample_file=file.path(repo, "logs", "stan-aus-sport-every-attempt-chain"))
