...
\newcommand{\stantime}{2 hour 56 minutes}
...
\newglossaryentry{onsight}{name=onsight, description={is a successful ascent of a route on the very first attempt, and without having any information about how the route should be climbed. If information is known about the route from other climbers, or by observing other climbers on the route, then it becomes a flash}}

\newglossaryentry{flash}{name=flash, description={is a successful ascent of a route or boulder problem on the very first attempt. It differs from onsight in that any amount of information about the route, including beta and observation of other climbers on the route is available to the climber when attempting a flash}}

\newglossaryentry{redpoint}{name=redpoint, description={is a successful ascent of a route or boulder problem on any attempt subsequent to the first attempt. If a successful ascent is logged as a redpoint it implies that climber has made previous attempts and/or ascents}}
...
There is one climber (Account ID: 2859555216) whose estimated session grade is much higher than all of the other climbers in the data set. Investigating this climber's ascent record reveals an extreme case of the problems with self-reported data and specifically with under-reporting of unsuccessful attempts. This climber's record contains 584 logged ascents: 429 onsight, 100 pinkpoint, 41 redpoint, 6 secondclean, 4 flash, 1 topropeonsight, 1 topropeflash, 2 allfreewithrest. Only the last two allfreewithrest ascents are counted as unsuccessful in our analysis. They were on a grade 25 and and a grade 27 respectively. It is clear from this record that this climber only records ascents that they view as having been a success. The 141 redpoint/pinkpoint ascents imply a minimum of 141 unreported unsuccessful attempts prior to redpoint for those routes. Even if this record is assumed to follow session logging conventions then that would imply that every redpoint/pinkpoint was achieved in a single session. If we removed allfreewithrest from our definition of unsuccessful attempt then this climber wouldn't have met our selection criteria as they would not have had a single unsuccessful attempt recorded in five years of climbing.
...
The authors would like to thank Simon Dale and Ulf Fuchslueger from \url{thecrag.com}, for providing access to the theCrag API (\url{https://www.thecrag.com/en/article/api}) so that the public ascent data could be downloaded programmatically for the analyses produced in this work. In addition we thank Dr Joseph Heled, Dr Tim Vaughan, Dr Michael Matschiner, and Daniel Krippner for helpful comments on and corrections to earlier versions of this manuscript.
