...
%DIF DEL ms-jqas.tex      Sun Mar  8 09:48:23 2026
%DIF ADD ms-jqas-r1.tex   Mon May 11 20:38:32 2026
...
%DIF -------
\newcommand{\ewbankslope}{2.10} %DIF > 
%DIF -------
...
For bouldering, the two common grading systems are the Vermin scale and the \DIFdelbegin \DIFdel{Fontainbleau }\DIFdelend \DIFaddbegin \DIFadd{Fontainebleau }\DIFaddend scale. The consensus view \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend that these two scales also have a correspondence in the higher grades, so that V11 $\equiv$ 8A, V12 $\equiv$ 8A+, $\dots$, V17 $\equiv$ 9A.
...
\begin{equation}
\Pr(y = 1) = p_{send} = \frac{e^{mC(t)}}{e^{mC(t)} + e^{mR}}\DIFaddbegin \DIFadd{,
...
\begin{equation}
\textrm{logit}^{-1}(x) = \frac{1}{1 + e^{-x}}\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation}
...
We analysed 20 datasets across three countries (New Zealand, Australia, Germany) using data from The Crag (\url{http://thecrag.com}), a popular online climbing logbook. In doing so we evaluate four different grading systems (\DIFdelbegin \DIFdel{Ewbanks}\DIFdelend \DIFaddbegin \DIFadd{Ewbank}\DIFaddend , French sport, UIAA, V-scale) and three styles of climbing involving different types of gear or no gear (Sport, Trad, Bouldering). Table \ref{datatable} summarises the datasets.
...
We describe two analyses in more detail. The first \DIFdelbegin \DIFdel{highlighted analysis uses self-reported data on }\DIFdelend \DIFaddbegin \DIFadd{uses the }\DIFaddend whole-history \DIFdelbegin \DIFdel{(meaning climber recorded every attempt) of ascents for 100 }\DIFdelend \DIFaddbegin \DIFadd{rating framework of \mbox{%DIFAUXCMD
\citet{coulom2008whole}}\hskip0pt%DIFAUXCMD
: all logged ascents (successes and failures) of $100$ }\DIFaddend Australian climbers of differing abilities \DIFaddbegin \DIFadd{are used jointly }\DIFaddend to estimate the fundamental slope parameter for the Ewbank climbing grade scale \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend sport climbing. We \DIFaddbegin \DIFadd{use ``whole-history'' to mean that the model uses every logged ascent in the analysis window, not that climbers necessarily logged every attempt they made; in practice, most climbers do not (we examine the consequences of this in Section~\ref{sec:every-attempt}). We }\DIFaddend repeated this analysis using the same \DIFdelbegin \DIFdel{criteria for selection (see Supplementary Material}\DIFdelend \DIFaddbegin \DIFadd{selection criteria (Section~4.1 below}\DIFaddend ) for climbers in New Zealand, resulting in an additional \DIFdelbegin \DIFdel{dataset }\DIFdelend \DIFaddbegin \DIFadd{data set }\DIFaddend of 89 climbers.


\begin{figure}
\centering
\includegraphics[width=\textwidth]{final-plot.png}
\caption{\small \DIFdelbeginFL \DIFdelFL{The posterior estimate }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Posterior distributions }\DIFaddendFL of the \DIFdelbeginFL \DIFdelFL{slope parameter ($d=e^m$) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{proportional difficulty increment per grade, $d = e^m$, }\DIFaddendFL for each of the 20 analyses. \DIFaddbeginFL \DIFaddFL{The horizontal axis (labelled ``Slope'') corresponds to $d$, the multiplicative factor by which the expected number of failures per success increases per unit of grade; $m$ denotes the latent slope parameter on the log-odds scale.}\DIFaddendFL }
\label{finalplot}
\end{figure}
...
\subsection{\DIFdelbegin \DIFdel{Estimating the slope of difficulty for the Ewbank climbing grade scale}\DIFdelend \DIFaddbegin \DIFadd{Whole-history Bayesian inference}\DIFaddend }

For the Australia-Sport analysis we used self-reported whole-history of ascents for $n=100$ climbers of differing abilities. All of these log books were self-reported on \url{http://thecrag.com}, a popular online climbing logbook. The total number of ascents analysed was \DIFdelbegin \DIFdel{$N=48,679$}\DIFdelend \DIFaddbegin \DIFadd{$N=48{,}679$}\DIFaddend . Most climbers do not self-report every attempt. To investigate the bias caused by climbers that don't log every failure we also converted all logbooks to a session logging form, in which only the single most successful ascent was chosen for each climber on each route on each day that climbing occurred.
\DIFdelbegin \DIFdel{In addition we only considered climbers that had }\DIFdelend \DIFaddbegin 

\DIFadd{Climbers were selected by applying two inclusion filters: (i) }\DIFaddend at least 30 \DIFdelbegin \DIFdel{attempts, with at least 1 }\DIFdelend \DIFaddbegin \DIFadd{logged attempts in the data window, and (ii) at least one }\DIFaddend explicit failure (i.e., at least one \gls{hangdog}, attempt, retreat or working). \DIFaddbegin \DIFadd{Among the climbers passing both filters, the $n=100$ with the largest number of logged ascents were retained for the Australia-Sport analysis; for the New Zealand sport analysis, all 89 eligible climbers (fewer than 100) were used. We deliberately did not impose stricter logging-quality filters at this stage so that the main estimates reflect the broad community of regularly active climbers; we report a separate analysis on the subset of climbers who appear to log every attempt in Section~\ref{sec:every-attempt}. }\DIFaddend The analysis took 1hr and 8 minutes on an iMac 3.6 GHz 8-Core Intel Core i9.

We implemented full Bayesian MCMC inference of the dynamic Bradley-Terry model in Stan \citep{carpenter2017stan}\DIFaddbegin \DIFadd{, using the No-U-Turn Sampler (NUTS) variant of Hamiltonian Monte Carlo}\DIFaddend . This code is available to the public domain at [URL removed for blind review].
...
\DIFaddbegin \paragraph{\DIFadd{Threshold sensitivity.}} \DIFadd{To address the concern that the $\geq\!30$ attempts inclusion threshold is somewhat arbitrary, we re-ran the Australia-Sport per-session analysis at attempt thresholds of $\geq\!50$ and $\geq\!100$, with the $n=100$ limit held fixed (Table~\ref{tab:threshold-sensitivity}). In all three runs the $n=100$ limit selects the same set of top-most-active climbers (each of whom logs hundreds of attempts and therefore comfortably clears all three thresholds), so the $N=48{,}679$ logged-ascent count and the posterior of $m$ are essentially identical (the small differences are MCMC sampling noise). This is informative but limited: it shows that the main result is not sensitive to whether a long-tailed group of marginally active climbers is included or excluded, but it does not directly probe what would happen if the limit itself were relaxed.
}

\DIFadd{For the latter question we appeal to the New Zealand sport analysis, where only $89$ climbers passed the $\geq\!30$/$\geq\!1$ filter and the $n=100$ limit therefore was not reached. The slope estimate $m = 0.80\ [0.76, 0.84]$ from this run is comparable in magnitude to the AUS estimate $m = 0.85\ [0.83, 0.86]$, consistent with the $n=100$ limit acting as a computational ceiling rather than a substantive analytical choice.
}
...
\hline
{\bf \DIFaddFL{Min.\ attempts}} & {\bf \DIFaddFL{$n$}} & {\bf \DIFaddFL{Ascents $N$}} & {\bf \DIFaddFL{$m$ posterior median }[\DIFaddFL{95\% CrI}]} & {\bf \DIFaddFL{$d=e^m$ median }[\DIFaddFL{95\% CrI}]} \\
\hline
\DIFaddFL{$\geq 30$  }& \DIFaddFL{$100$ }& \DIFaddFL{$48{,}679$ }& \DIFaddFL{$0.85\ [0.83, 0.86]$ }& \DIFaddFL{$2.33\ [2.29, 2.37]$ }\\
\DIFaddFL{$\geq 50$  }& \DIFaddFL{$100$ }& \DIFaddFL{$48{,}679$ }& \DIFaddFL{$0.85\ [0.83, 0.86]$ }& \DIFaddFL{$2.33\ [2.29, 2.37]$ }\\
\DIFaddFL{$\geq 100$ }& \DIFaddFL{$100$ }& \DIFaddFL{$48{,}679$ }& \DIFaddFL{$0.85\ [0.83, 0.86]$ }& \DIFaddFL{$2.33\ [2.29, 2.37]$ }\\
\hline
\end{tabular}
\caption{\DIFaddFL{Sensitivity of the slope estimate to the per-climber attempt threshold for the Australia-Sport per-session analysis, holding the $n=100$ limit fixed. The limit selects the same top-$100$ most-active climbers in all three runs; each of these climbers exceeds the highest threshold ($\geq\!100$) by a large margin, so the threshold has no effect on the selected sample. The estimates differ only in MCMC stochasticity (max $\hat{R} = 1.004$, min $n_{\mathrm{eff}} \geq 1{,}359$, $0$ divergent transitions in all runs).}}
\label{tab:threshold-sensitivity}
\end{table}

\DIFaddend Figure \ref{nz_ascents} shows the estimated grade through time plot between August 2016 and July 2021 inclusive for 89 climbers that fulfilled our selection criteria for the New Zealand Sport data set. The second panel reports the posterior distribution of the slope parameter which was jointly estimated.
...
\DIFaddbegin \subsection{\DIFadd{Every-attempt logger subset}}
\label{sec:every-attempt}

\DIFadd{Selective logging of failures is the largest single threat to the interpretation of the slope estimate $m$ (see Discussion, Section~6). To bound this bias empirically, we re-ran the Australia-Sport per-session analysis on the subset of climbers who appear to log every attempt rather than only successful sends. The selection rule was that of the main analysis (Section~4.1) augmented by two additional logging-quality filters: (i) at most $5\%$ of a climber's logged ascents are of the generic ambiguous ``tick'' type that does not unambiguously imply a clean send; and (ii) at most $5\%$ are }\emph{\DIFadd{lonely redpoints}}\DIFadd{, where a lonely redpoint is a redpoint of a route for which no other ascent of that route by that climber has been logged. Both criteria measure the completeness of a climber's history: a climber who logs every attempt produces near-zero lonely redpoints (because every redpoint is preceded in the log by at least one failed attempt on the same route) and near-zero generic ticks (because they record success or failure unambiguously).
}

\DIFadd{More climbers than the $n=100$ limit passed both logging-quality filters; we therefore retained the top-$100$ most-active among them, comprising $N = 26{,}578$ per-session ascents (compared with $N=48{,}679$ for the main analysis using the same $n=100$ limit, illustrating that the most-prolific climbers under selective-logging conventions accumulate substantially more logged events than the most-prolific every-attempt loggers). The posterior estimate of the slope was $m = 0.76\ [0.74, 0.79]$, equivalent to $d = e^m = 2.15\ [2.10, 2.19]$ times more failed attempts per success per grade increment. Diagnostics for this analysis match the main run quality: maximum $\hat{R} = 1.01$ across all $6{,}002$ monitored parameters, minimum bulk $n_{\mathrm{eff}} = 848$, and zero divergent transitions.
}

\DIFadd{This estimate provides an empirical complement to the main figure of $d \approx 2.33$. The shift is downwards but modest: the every-attempt-logger estimate is still above $2$. The direction of the shift is nonetheless consistent with the hypothesis that under-reporting of failed attempts in less complete logbooks inflates the apparent slope, since failures on easier routes are more likely to be omitted than failures on harder routes \mbox{%DIFAUXCMD
\citep{scarff2020estimation}}\hskip0pt%DIFAUXCMD
. We caution that ``apparent every-attempt logging'' is itself an inferred property of the logbook rather than a guarantee, and that the threshold values used to define the subset (at most $5\%$ generic ticks and at most $5\%$ lonely redpoints) are themselves judgement calls; the estimate should therefore be read as one empirical anchor rather than as a definitive correction. We interpret the main figure $d \approx 2.33$ as a community-wide average that is plausibly biased upwards by $\sim$10\% due to selective failure logging, and the every-attempt-logger figure $d \approx 2.15$ as a lower-bias estimate suggesting that the true multiplicative difficulty per grade in Ewbank-system sport climbing lies somewhere in the neighbourhood of $2.1$--$2.3$.
}

\DIFaddend \subsection{Estimating the slope of total successful ascents versus grade in community-wide ascent data}

Figure \ref{fig.oneill} shows the whole-community counts of successful ascents by grade for \DIFdelbegin \DIFdel{Australian }\DIFdelend \DIFaddbegin \DIFadd{Australia }\DIFaddend and New Zealand for both sport climbs and bouldering. \DIFdelbegin \DIFdel{The slope of these curves range between 0.7 and 0.79, conforming remarkably well with the more detailed Bayesian analysis above. This data is not affected by bias in }\DIFdelend \DIFaddbegin \DIFadd{Fitted on the log scale, these curves have slopes in the range $0.7$ to $0.79$. We caution that this analysis is not a validation of the slope parameter $m$ estimated from the dynamic Bradley-Terry model: although the curves are unaffected by }\DIFaddend under-reporting of failed attempts, \DIFdelbegin \DIFdel{but will be affected by systematic variation in }\DIFdelend \DIFaddbegin \DIFadd{the number of successful ascents at each grade is jointly determined by (i) the difficulty of routes at that grade, (ii) the number of routes available at that grade in each region, and (iii) }\DIFaddend the number of attempts \DIFdelbegin \DIFdel{made by }\DIFdelend the community as a whole \DIFdelbegin \DIFdel{at each grade. }\DIFdelend \DIFaddbegin \DIFadd{chooses to make at that grade. The first of these is the quantity $m$ describes, but the latter two are confounded with it and we have no independent estimate of either. We therefore present these curves as a complementary descriptive observation rather than as evidence in support of the Bayesian estimates, and we make no quantitative comparison between the two slopes.
}\DIFaddend 
