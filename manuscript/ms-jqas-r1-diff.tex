...
%DIF ADD ms-jqas-r1.tex   Mon May 11 19:39:37 2026
...
\hline
\end{tabular}
\caption{\DIFaddFL{Sensitivity of the slope estimate to the per-climber attempt threshold for the Australia-Sport per-session analysis, holding the $n=100$ cap fixed. The cap binds the same top-$100$ most-active climbers in all three runs; each of these climbers exceeds the highest threshold ($\geq\!100$) by a large margin, so the threshold has no effect on the selected sample. The estimates differ only in MCMC stochasticity (max $\hat{R} = 1.004$, min $n_{\mathrm{eff}} \geq 1{,}359$, $0$ divergent transitions in all runs).}}
