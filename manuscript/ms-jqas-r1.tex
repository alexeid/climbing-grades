...
Climbers were selected by applying two inclusion filters: (i) at least 30 logged attempts in the data window, and (ii) at least one explicit failure (i.e., at least one \gls{hangdog}, attempt, retreat or working). Among the climbers passing both filters, the $n=100$ with the largest number of logged ascents were retained for the Australia-Sport analysis; for the New Zealand sport analysis, all 89 eligible climbers (fewer than 100) were used. We deliberately did not impose stricter logging-quality filters at this stage so that the headline estimates reflect the broad community of regularly active climbers; we report a separate analysis on the subset of climbers who appear to log every attempt in Section~\ref{sec:every-attempt}. The analysis took 1hr and 8 minutes on an iMac 3.6 GHz 8-Core Intel Core i9.
