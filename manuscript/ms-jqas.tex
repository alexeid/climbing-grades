...
For bouldering, the two common grading systems are the Vermin scale and the Fontainbleau scale. The consensus view as that these two scales also have a correspondence in the higher grades, so that V11 $\equiv$ 8A, V12 $\equiv$ 8A+, $\dots$, V17 $\equiv$ 9A.
...
We implemented full Bayesian MCMC inference of the dynamic Bradley-Terry model in Stan \citep{carpenter2017stan}. This code is available to the public domain at [URL removed for blind review].
