...
\title{Response to Reviewers - Manuscript ID DGJQAS.2026.0040 ``Bayesian inference of the climbing grade scale''}
\author{}
\date{}
...
We must also apologise that the supplementary material was inadvertently omitted from the original submission. We believe this oversight partially accounts for the reviewer's concerns about the level of detail provided on the model and inference procedure, since some of the supporting methodological material was intended to live in the supplement but was not in fact available to the reviewer. The supplementary material has been substantially expanded for this revision and is included alongside the revised manuscript; we have additionally expanded the main article itself so that all methodological points the reader needs to evaluate the work are stated in the main text, with the supplement providing additional detail rather than being essential to any methodological claim.
...
\end{review}
\response We have added a threshold-sensitivity table to the revised manuscript (Table~3, in the ``Threshold sensitivity'' paragraph of Section~5.1) reporting posterior summaries for $m$ at attempt thresholds $\geq 30$, $\geq 50$ and $\geq 100$, with the $n=100$ limit held fixed.

We report a finding worth flagging explicitly: in all three runs the $n=100$ limit selects the same set of top-most-active climbers, each of whom logs hundreds of attempts and therefore comfortably clears all three thresholds. The selected sample (and hence the posterior of $m$) is therefore identical across the three thresholds up to MCMC noise. This shows directly that the main result is unaffected by the inclusion or exclusion of any tail of less-active climbers, but it does not by itself probe what would happen if the $n=100$ limit were relaxed.

For that latter question we appeal to the existing New Zealand sport analysis, where only $89$ climbers passed the $\geq 30$/$\geq 1$ filter and the $n=100$ limit therefore was not reached. The slope estimate $m = 0.80\ [0.76, 0.84]$ from this run is comparable to the AUS estimate $m = 0.85\ [0.83, 0.86]$, supporting the interpretation of the limit as a computational ceiling rather than a substantive analytical choice. We have stated this comparison explicitly in the manuscript so the reader can see that the limit has little effect on the inference.
...
We should be transparent about the relationship between the original ``data not shown'' claim and the revised analysis. The original informal comparison was based on a small number ($n=2$) of climbers personally known to the authors as thorough attempt-loggers. The revised analysis instead applies an indirect, data-driven proxy to the broader population: at most $5\%$ ambiguous ``tick'' ascents and at most $5\%$ lonely redpoints in a climber's logbook. The two are different ways of capturing the same underlying concept (thorough attempt-logging), and select only partially overlapping populations: of the $n=100$ climbers in the main Australia-Sport analysis and the $n=100$ in the new every-attempt-logger subset, only $33$ are common to both ($67$ are in the main set but failed the proxy filter, and $67$ pass the proxy filter but had insufficient ascent counts to enter the main top-$100$). The new proxy therefore selects a meaningfully different population while still capturing the spirit of the original claim by isolating climbers whose logbook structure is consistent with logging every attempt.

For the Australia-Sport per-session data set, more than $100$ climbers passed both proxy filters, so we retained the top-$100$ most-active among them (consistent with the $n=100$ limit used in the main analysis), comprising $N = 26{,}578$ per-session ascents. The posterior estimate on this subset was $m = 0.76$ [95\% CrI: $0.74, 0.79$], corresponding to $d = e^m = 2.15$ [95\% CrI: $2.10, 2.19$]. As anticipated, this estimate sits meaningfully below the main estimate of $d \approx 2.33$, providing empirical support for the direction of the under-reporting-bias claim. We note that the every-attempt-logger estimate is still above $2$ rather than below $2$ as the original ``data not shown'' sentence asserted; the revised manuscript (new Section~5.2) now states this directly and interprets the main estimate as plausibly biased upwards by $\sim$10\% rather than substantially overstated. Convergence diagnostics for this run are clean (maximum $\hat{R} = 1.01$, minimum $n_{\mathrm{eff}} = 848$, zero divergent transitions across $6{,}002$ parameters).
